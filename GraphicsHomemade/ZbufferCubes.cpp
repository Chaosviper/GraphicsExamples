...
int ZbufferCubes::LoadResources(){
	...
	// ** LOAD SECOND PRECOMPILED PIXEL SHADER
#if _DEBUG
	...
	res = g_d3dDevice->CreatePixelShader(pixelShaderBlob->GetBufferPointer(), pixelShaderBlob->GetBufferSize(), nullptr, &D11_pixelShaderMonocolored);
	if (FAILED(res))
	{
		return -1;
	}
...
int ZbufferCubes::implementedRender(){
	...
	XMMATRIX worldUpdatedCoord = XMMatrixMultiply(XMMatrixRotationY(cubeAngleCoord), XMMatrixTranslation(0.0f, 0.0f, 5.0f));
